...
////    @Mapping(source = "request.patientId", target = "patientId")
////    @Mapping(source = "entity.patient.userId", target = "patientId")
...
public interface LabOrderMapper {
    ...
    // Map DTO Long IDs to entity relations (User) using a lightweight reference
    @Mapping(source = "patientId",   target = "patient",   qualifiedByName = "userRefFromId")
    @Mapping(source = "clinicianId", target = "clinician", qualifiedByName = "userRefFromId")

    @Mapping(source = "orderDate", target = "orderDate")
    @Mapping(source = "priority",  target = "priority")
    @Mapping(source = "testIds",   target = "testIds")

    // Defaults / ignored fields
    @Mapping(target = "status",        constant = "Ordered")
    @Mapping(target = "specimens",     ignore = true)
    @Mapping(target = "audit",         ignore = true)
    @Mapping(target = "workflows",     ignore = true)
    ...
    // LabOrder -> LabOrderResponse (explicit for clarity & to avoid path resolution issues)
    @Mapping(source = "orderId", target = "orderId")
    // Use expression to safely reach nested properties
    @Mapping(target = "patientId",
            expression = "java(entity.getPatient() != null ? entity.getPatient().getUserId() : null)")
    @Mapping(target = "clinicianId",
            expression = "java(entity.getClinician() != null ? entity.getClinician().getUserId() : null)")
    @Mapping(source = "orderDate", target = "orderDate")
    @Mapping(source = "priority",  target = "priority")
    @Mapping(source = "status",    target = "status")
    @Mapping(source = "testIds",   target = "testIds")
    ...
    // Helper to create a lightweight User reference from an ID (no DB hit)
    @Named("userRefFromId")
    default User userRefFromId(Long id) {
